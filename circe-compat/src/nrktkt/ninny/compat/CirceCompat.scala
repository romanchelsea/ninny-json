...
trait NinnyToCirce {
  ...
  implicit def toSomeJsonEncoder[A](implicit toSomeJson: ToSomeJson[A]): Encoder[A] =
    new Encoder[A] {
  ...
  implicit def fromJsonDecoder[A](implicit fromJson: FromJson[A]): Decoder[A] = new Decoder[A] {
    override def apply(c: HCursor): Result[A] = fromJson.from(c.root.value) match {
      case Success(value) => Right(value)
      case Failure(_)     => Left(DecodingFailure("foo", { List.empty }))
    }
  }

  implicit def asCirce(json: JsonValue): Json = json match {
    case JsonNull                            => Json.Null
    case JsonBoolean(value)                  => Json.fromBoolean(value)
    case JsonString(value)                   => Json.fromString(value)
    case nrktkt.ninny.ast.JsonDouble(value)  => Json.fromDoubleOrNull(value)
...
trait CirceToNinny {
  ...
  implicit def encoderToSomeJson[A](implicit encoder: Encoder[A]): ToSomeJson[A] = new ToSomeJsonValue[A, JsonValue] {
  ...
  implicit def asNinny(json: Json): JsonValue = json match {
    ...
    case _ if json.isArray   => JsonArray(json.asArray.get.map(asNinny).toIndexedSeq)
    ...
    case _ if json.isObject =>
      nrktkt.ninny.ast.JsonObject(json.asObject.get.toMap.map {
        case (k, v) => (k, asNinny(v))
      })
  }
}
...
object CirceCompat extends NinnyToCirce with CirceToNinny